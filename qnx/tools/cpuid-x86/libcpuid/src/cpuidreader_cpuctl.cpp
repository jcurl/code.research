...
class cpuid_cpuctl_file {
 public:
  /// @brief Open '/dev/cpuctl<core>'.
  ///
  /// If the file could not be opened, this object's boolean operator will
  /// return false. Get the error with the error() function.
  ///
  /// @param core The core to open.
  cpuid_cpuctl_file(unsigned int core) {
    std::string path = "/dev/cpuctl" + std::to_string(core);
    ...
    core_ = core;
    cpuid_ = cpuid_cpuctl{fd_};
  }

  cpuid_cpuctl_file(const cpuid_cpuctl_file&) = delete;

  cpuid_cpuctl_file(cpuid_cpuctl_file&& other) noexcept
      : fd_{std::exchange(other.fd_, -1)},
        core_{std::exchange(other.core_, 0)},
        error_{std::exchange(other.error_, 0)},
        cpuid_{std::exchange(other.cpuid_, cpuid_cpuctl{})} {}

  auto operator=(const cpuid_cpuctl_file&) -> cpuid_cpuctl_file& = delete;

  auto operator=(cpuid_cpuctl_file&& other) noexcept -> cpuid_cpuctl_file& {
    cpuid_cpuctl_file{std::move(other)}.swap(*this);
    return *this;
  }
  ...
  [[nodiscard]] auto cpuid() -> cpuid_cpuctl& { return cpuid_; }
  ...
  auto swap(cpuid_cpuctl_file& other) noexcept -> void {
...
auto cpuidreader_cpuctl::has_cpuid() -> bool {
  if (!ctx_ || !(*ctx_) || (ctx_.use_count() == 1 && ctx_->core() != 0)) {
    ...
    ctx_ = std::make_shared<detail::cpuid_cpuctl_file>(0);
  }
...
auto cpuidreader_cpuctl::cpuid(cpuidreg eax, cpuidreg ecx)
...
auto cpuidreader_cpuctl::enable_core(unsigned int core)
  ...
  if (ctx_ && *ctx_) {
    ...
    if (ctx_->core() == core) {
      ...
      return std::make_unique<
          detail::cpuid_basic_ctx<detail::cpuid_cpuctl_file>>(ctx_);
    }
  ...
  ctx_ = std::make_shared<detail::cpuid_cpuctl_file>(core);
  ...
  return std::make_unique<detail::cpuid_basic_ctx<detail::cpuid_cpuctl_file>>(
      ctx_);
}
