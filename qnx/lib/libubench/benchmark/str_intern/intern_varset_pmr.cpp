...
class intern_var_set_pmr::interned {
 private:
 ...
 private:
  ...
  alloc_s_t alloc_s_{};
  string_block_t strings_{&alloc_s_};
...
// NOLINTNEXTLINE(bugprone-easily-swappable-parameters)
intern_var_set_pmr::intern_var_set_pmr(
    std::size_t buckets, std::size_t max_buckets) {
...
auto intern_var_set_pmr::intern(std::string_view str) -> const std::string& {
  return interned_->intern(str);
}

auto intern_var_set_pmr::size() const -> unsigned int {
  return interned_->size();
}

[[nodiscard]] auto intern_var_set_pmr::max_load_factor() const -> float {
  return interned_->max_load_factor();
}

auto intern_var_set_pmr::max_load_factor(float ml) -> void {
  if (ml <= 0.01) {
